...
const STATUS_COLORS = {
  PENDING: { bg: "var(--color-warning)", text: "var(--color-text-primary)" },
  CONFIRMED: { bg: "var(--color-brand)", text: "#ffffff" },
  SHIPPED: { bg: "var(--color-purple)", text: "#ffffff" },
  DELIVERED: { bg: "var(--color-success)", text: "#ffffff" },
  CANCELLED: { bg: "var(--color-danger)", text: "#ffffff" },
  REFUND_REQUESTED: { bg: "var(--color-orange)", text: "#ffffff" },
  REFUND_PROCESSING: { bg: "var(--color-warning)", text: "#ffffff" },
  REFUNDED: { bg: "var(--color-slate)", text: "#ffffff" },
};
    ...
    return (
      <div className="mt-4 space-y-3">
        {history.map((entry, idx) => {
            const color = STATUS_COLORS[entry.newStatus] || { bg: "var(--border-color)", text: "var(--text-primary)" };
            return (
              <div key={idx} className="flex items-start gap-3">
                <div className="flex flex-col items-center">
                  <div className="h-3 w-3 rounded-full" style={{ backgroundColor: color.bg }} />
                  {idx < history.length - 1 && <div className="mt-1 h-8 w-0.5 bg-gray-200 dark:bg-gray-700" />}
                </div>
                <div className="flex-1 pb-2">
                  ...
                  {entry.note && <p className="text-xs text-gray-400 italic">Note: {entry.note}</p>}
                </div>
              </div>
            );
          })}
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Manage orders, update status, and view details.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          {orders.length > 0 && page === 0 && (
            <span className="text-xs text-gray-500">CSV exports current page only</span>
          )}
        </div>
      </div>
        ...
        <div className="grid gap-4 md:grid-cols-5">
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:placeholder-gray-400"
            />
          </div>
        </div>
      </div>
          ...
          <table className="admin-table text-left text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                <th className="pb-3 pr-4 font-medium text-gray-500 dark:text-gray-400">Order ID</th>
                <th className="pb-3 pr-4 font-medium text-gray-500 dark:text-gray-400">Customer</th>
                <th className="pb-3 pr-4 font-medium text-gray-500 dark:text-gray-400">Items</th>
                <th className="pb-3 pr-4 font-medium text-gray-500 dark:text-gray-400">Total</th>
                <th className="pb-3 pr-4 font-medium text-gray-500 dark:text-gray-400">Payment</th>
                ...
                <th className="pb-3 pl-4 font-medium text-gray-500 dark:text-gray-400">Actions</th>
              </tr>
            </thead>
            <tbody>
              {orders.map((order) => {
                ...
                return (
                  <tr key={order.id} className="border-b border-gray-100 dark:border-gray-800 last:border-0">
                    <td className="py-4 pr-4 font-mono text-gray-900 dark:text-white">{order.id}</td>
                    <td className="py-4 pr-4">
                      <div className="flex items-center gap-3">
                        <div
                          className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-blue-100 text-sm font-semibold text-blue-700 dark:bg-blue-900 dark:text-blue-200"
                        >
                          {(order.customerEmail || order.user?.email || "U")[0]?.toUpperCase()}
                        </div>
                        <div>
                          <div className="font-medium text-gray-900 dark:text-white">{order.customerEmail || order.user?.email || "N/A"}</div>
                          {order.user?.username && (
                            <div className="text-xs text-gray-500 dark:text-gray-400">{order.user.username}</div>
                          )}
                        </div>
                      </div>
                    </td>
                    <td className="py-4 pr-4 text-gray-700 dark:text-gray-300">
                      {itemCount > 0 ? (
                        <span>
                          ...
                          {firstItem ? <span className="block text-xs text-gray-500">First: {firstItem}</span> : null}
                        </span>
                      ) : (
                        "—"
                      )}
                    ...
                    <td className="py-4 pr-4 text-gray-700 dark:text-gray-300">
                      <span className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2 py-1 text-xs font-medium dark:bg-gray-800">
                        {order.paymentMethod || "—"}
                      </span>
                    </td>
                    <td className="py-4 pr-4">
      ...
      {detailLoading && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900">
            ...
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">Loading order…</p>
          </div>
        </div>
      )}
...
{selectedOrder && !detailLoading && (
         <div className="fixed inset-0 z-50 flex justify-end" role="dialog" aria-modal="true" aria-labelledby="order-drawer-title">
           ...
           <div ref={orderDrawerRef} className="relative h-full w-full max-w-2xl overflow-y-auto bg-white shadow-2xl dark:bg-gray-900">
             <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4 dark:border-gray-800 dark:bg-gray-900">
               <h3 id="order-drawer-title" className="text-lg font-semibold text-gray-900 dark:text-white">Order #{selectedOrder.id}</h3>
               <button
            ...
            <div className="space-y-6 px-6 py-6">
              <section>
                <h4 className="mb-2 text-sm font-semibold text-gray-500 uppercase tracking-wide">Order Info</h4>
                <div className="grid gap-4 md:grid-cols-3">
                  <div>
                    <p className="text-xs text-gray-500">Order ID</p>
                    <button
                      onClick={() => copyToClipboard(`#${selectedOrder.id}`)}
                      className="flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-mono dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"
                    >
                      #{selectedOrder.id}
                      <span className="text-xs text-blue-600 dark:text-blue-400">Copy</span>
                    </button>
                  </div>
                  <div>
              ...
              <section>
                <h4 className="mb-2 text-sm font-semibold text-gray-500 uppercase tracking-wide">Customer</h4>
                <div className="flex items-center gap-4">
                  <div className="flex h-12 w-12 items-center justify-center rounded-full bg-blue-100 text-lg font-semibold text-blue-700 dark:bg-blue-900 dark:text-blue-200">
              ...
              <section>
                <h4 className="mb-2 text-sm font-semibold text-gray-500 uppercase tracking-wide">Delivery Address</h4>
                <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
                  {selectedOrder.deliveryAddress ? (
                    <div className="text-sm text-gray-800 dark:text-gray-200">
                      ...
                      {selectedOrder.deliveryAddress.phone && <p className="mt-1 text-gray-600 dark:text-gray-400">Phone: {selectedOrder.deliveryAddress.phone}</p>}
                    </div>
                  ) : (
                    <p className="text-sm text-gray-500 dark:text-gray-400">No address on file.</p>
                  )}
                </div>
              </section>

              <section>
                <h4 className="mb-2 text-sm font-semibold text-gray-500 uppercase tracking-wide">Items</h4>
                  ...
                  <table className="w-full text-left text-sm">
                    <thead>
                      <tr className="border-b border-gray-200 dark:border-gray-700">
                        ...
                        <th className="pb-2 font-medium text-gray-500 dark:text-gray-400">Unit Price</th>
                        <th className="pb-2 text-right font-medium text-gray-500 dark:text-gray-400">Line Total</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                      {(selectedOrder.orderItems || []).map((item) => (
                        <tr key={item.id}>
                          ...
                          <td className="py-3 pr-3 text-gray-700 dark:text-gray-300">{item.quantity}</td>
                          <td className="py-3 pr-3 text-gray-700 dark:text-gray-300">{formatCurrency(item.unitPrice)}</td>
                          <td className="py-3 text-right font-medium text-gray-900 dark:text-white">{formatCurrency(item.lineTotal)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
...
<div className="mb-3">
                   <span className="inline-flex rounded-full px-3 py-1 text-xs font-semibold" style={{ backgroundColor: (STATUS_COLORS[selectedOrder.status] || { bg: "var(--border-color)", text: "var(--text-primary)" }).bg, color: (STATUS_COLORS[selectedOrder.status] || { bg: "var(--border-color)", text: "var(--text-primary)" }).text }}>
              ...
              <section>
                <h4 className="mb-3 text-sm font-semibold text-gray-500 uppercase tracking-wide">Admin Actions</h4>
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="rounded-xl border border-gray-200 p-4 dark:border-gray-700">
                    <h5 className="mb-3 text-sm font-semibold text-gray-900 dark:text-white">Update Order Status</h5>
                    {allowedNextStatuses.length === 0 ? (
                      <p className="text-sm text-gray-500 dark:text-gray-400">No further actions available for this order.</p>
                    ) : (
                      <div className="space-y-3">
                        <div>
                          <label className="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-300">New Status</label>
                          <select
                        ...
                        {statusForm.status === "SHIPPED" && (
                          <div className="grid gap-3 md:grid-cols-2">
                            <div>
                              <label className="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-300">Tracking Number</label>
                              <input
                                type="text"
                            ...
                            <div>
                              <label className="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-300">Courier Name</label>
                              <input
                                type="text"
                        ...
                        <div>
                          <label className="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-300">Note (optional)</label>
                          <textarea
                  ...
                  <div className="rounded-xl border border-gray-200 p-4 dark:border-gray-700">
                    <h5 className="mb-3 text-sm font-semibold text-gray-900 dark:text-white">Communication</h5>
                    <button
                      onClick={handleResendEmail}
                      className="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-800 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-800"
                    >
